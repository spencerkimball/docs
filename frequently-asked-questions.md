...
- **Isolation:** Transactions in CockroachDB implement the strongest ISO isolation level: serializable (SERIALIZABLE). This means that transactions will never result in anomalies. For a detailed discussion of isolation in CockroachDB transactions, see [Serializable, Lockless, Distributed: Isolation in CockroachDB](https://www.cockroachlabs.com/blog/serializable-lockless-distributed-isolation-cockroachdb/).
